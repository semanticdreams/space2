...
(fn append-handles [target returned context]
  ...
  (var max-index 0)
  (var valid-keys? true)
  (var handle-error nil)
  (each [key handle (pairs returned)]
    (if (and (= (type handle) "table") (= (type handle.unregister) "function"))
        (record-handle! handle)
        (when (not handle-error)
          (set handle-error (if (= (type handle) "table")
                                (.. context " returned handle without unregister")
                                (.. context " returned non-table handle")))))
    (set count (+ count 1))
    (if (and (= (type key) "number") (= key (math.floor key)) (> key 0))
        (set max-index (math.max max-index key))
        (set valid-keys? false)))
  (when handle-error
    (error handle-error))
  (assert (> count 0)
          (.. context " must return a non-empty sequential table of registration handles"))
