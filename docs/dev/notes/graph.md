...
- `graph/extensions/builtins/`: built-in graph extension descriptors install loaders that adapt owning stores/systems into graph nodes on demand via `load-by-key`. Entity loaders adapt entity stores; LLM loaders adapt the LLM store; world activity and surface loaders adapt `world-manager` and `WorldData`.
