...
- [ ] **Step 6: Implement `assets/lua/text-editing-port.fnl`.** Implement port-equivalent eager/logical navigation behavior using current `text-state.fnl` behavior as the reference, but do not modify `text-state.fnl` in this task. Keep helper functions private, including whitespace detection, line-start index calculation, line length clamping, preferred-column storage on the input object, and backend detection. The actual extraction/removal from `text-state.fnl` happens in Task 2.
